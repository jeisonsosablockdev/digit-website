...
collect_scope() {
  local name="$1"
  case "$name" in
    architecture.md|authority-model.md|state-machine.md|threat-model.md)
      echo "core"
      ;;
...

{
  ...
  echo '- Core architecture: `architecture.md`, `authority-model.md`, `state-machine.md`, `threat-model.md`'
